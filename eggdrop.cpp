/*
Suppose that we wish to know which story in a k-story building are safe to drop eggs from, and which will cause the eggs to break on landing. We make a few assumptions:
...
If only one egg is available and we wish to be sure of obtaining the right result, the experiment can be carried out in only one way. Drop the egg from the first-floor window; if it survives, drop it from the second floor window. Continue upward until it breaks. In the worst case, this method may require k droppings. Suppose n eggs are available. What is the least number of egg-droppings that is guaranteed to work in all cases?
 */
#include <iostream>
#include <vector>
#include <functional>
  ...
  for (uint32_t i = 2; i < egg_num+1; ++i) {
    for (uint32_t j = 2; j < story_num+1; ++j) {
      ...
      for (uint32_t k = 1; k < j+1; ++k) {
        const uint32_t tmp = 1 + max(dp[i-1][k-1], dp[i][j-k]); // 1) If the egg breaks after dropping from k-th floor, then we only need to check for floors lower than k with remaining eggs so the problem reduces to k-1 floors and n-1 eggs; 2) If the egg doesn't break after dropping from the k-th floor, then we only need to check for floors higher than k so the problem reduces to j-k floors and n eggs.
...
uint32_t eggdrop_recur(const uint32_t n, const uint32_t k) {
  function<uint32_t(const uint32_t,const uint32_t)> dfs =
    ...
    for (uint32_t i = 1; i < story_num+1; ++i) {
      uint32_t tmp = max(dfs(egg_num-1, i-1), dfs(egg_num, story_num-i));
      if (res < res)
        res = tmp;
    }
...
int main(int argc, char** argv) {
  ...
  cout << "DP: minimum number of trials with " << egg_num << " eggs and " << story_num << " floors: " << eggdrop_dp(egg_num, story_num) << endl;
