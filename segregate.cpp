...
2) Create a temporary array sumleft[] of size n. Store the sum of all elements from a[0] to a[i] in sumleft[i]. This can be done in O(n) time.
...
// Fill sumleft array and get min and max values in it. Consider 0 values in arr[] as -1
  ...
  // Check if this j-i is more than maxsize. The optimum size of hash will be max-min+1 as these many different values of sumleft[i] are possible. Since we use optimum size, we need to shift all values in sumleft[] by min before using them as an index in hash[].
