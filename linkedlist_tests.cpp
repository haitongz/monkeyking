...
ListNode* middle_iter(ListNode* head) {
  if (!head)
    return nullptr;
...
void sortedRemoveDup(ListNode* head) {
  if (!head) {
#ifdef _DEBUG_
    cerr << "Null head!" << endl;
#endif
...
void quickSort(ListNode** head_ref) {
  function<ListNode*(ListNode*,ListNode*)> solve =
    [&](ListNode* curr_head, ListNode* end) { // sorting happens exclusive of the end node
    // base condition
    if (!curr_head || curr_head == end)
        return curr_head;

    ListNode* new_head = nullptr, *new_end = nullptr;
 ...
 */
void delList_iter(ListNode** head) {
  if (!head) {
#ifdef _DEBUG_
    cerr << "Null head!" << endl;
#endif
...
void delList_recur(ListNode** head) {
  if (!head) {
#ifdef _DEBUG_
    cerr << "Null head!" << endl;
#endif
...
Time Complexity: O(n)
 */
void delLesserNodes_recur(ListNode** head_ref) {
