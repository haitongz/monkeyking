...
vector<uint32_t> numbersWith5(const uint32_t n) {
  if (n < 5)
    return {};

  ostringstream oss;
  oss << n;
  const string number = oss.str();
  const uint32_t len = number.length();
  string to_ext = "";
  vector<uint32_t> res;

  function<void(const uint32_t,const bool)> backtrack =
    [&](const uint32_t idx, const bool there_is_5) {
    if (idx == len) {
      res.push_back(atoi(to_ext.c_str()));
      return;
      ...
      for (uint8_t i = 0; i < 10; ++i) {
        ...
        if (atoi(to_ext.c_str()) <= atoi(new_num.c_str())) {
          backtrack(idx+1, (i == 5) ? true : there_is_5);
        }
        to_ext.pop_back();
      }
...
int main(int argc, char** argv) {
  ...
  vector<uint32_t> res = numbersWith5(n);
