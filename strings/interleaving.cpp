...
bool isInterleaving_recur(const string& s1, const string& s2, const string& s3) {
  const uint len1 = s1.length(), len2 = s2.length(), len3 = s3.length();
  if (len1+len2 != len3)
    return false;

  function<bool(const uint,const uint,const uint)> solve =
    [&](const uint idx1, const uint idx2, const uint idx3) {
...
vector<string> interleavings(const string& s1, const string& s2) {
  ...
  function<void(const uint,const uint,const string&)> solve =
    [&](const uint idx1, const uint idx2, const string& to_ext) {
    if (idx1 == len1) {
