 ...
 */
double maxProfit4(const vector<double>& prices, uint32_t k) {
  const uint32_t n = prices.size();
  if (n < 2 || k < 1)
    return 0.0;
  if (k > n)
...
/*  uint32_t N;
  ...
  cout << "Max profit with 1 transaction: " << maxProfit(prices) << endl;
  cout << "Max profit with 1 transaction: " << maxProfit4(prices, 1) << endl;
  cout << "Max profit with any transactions: " << maxProfit2(prices) << endl;
  cout << "Max profit with 2 transactions: " << maxProfit3(prices) << endl;
  cout << "Max profit with 2 transactions: " << maxProfit4(prices, 2) << endl;
