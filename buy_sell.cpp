...
double maxProfit_1Trans(const double prices[], const uint n) {
  ...
  for (uint i = 1; i < n; ++i) { // time series when scanning from first to last
    if (prices[i] < lowest_px) {
      lowest_px = prices[i]; // no need to check max_profit because you have to buy first sell later
    } else {
      const double maxpft_ati = prices[i]-lowest_px;
      ret = max(ret, maxpft_ati);
    }
  }
...
/*
If a trader is allowed to buy a share first and sell'em all in one day (buy->buy->buy->sell).
Given stock prices throughout day, find out maximum profit that a share trader could have made.
Input:  px[] = {10, 22, 5, 75, 65, 70}
Output: 65+53+70+5=193
Buy at price 10, 22 and 5, then sell at 75. Buy at 65 then sell at 70
 */
...
4 2
 */
const uint MAX_N = 5001;
uint prices[MAX_N]; // stock prices

pair<int,int> buylowBuylower(const uint N) {
  if (N < 2)
    return {-1,-1};

  uint longest_subseq_cnt[MAX_N]; // how many longest subseqs with index i
  uint longest_subseq_len[MAX_N]; // longest_subseq_len[i]: how long longest subseqs with index i
