 ...
 */
vector<uint32_t> naive(const string& txt, const string& patt) {
  const uint32_t N = txt.length(), M = patt.length();
  if (!N || !M || N < M)
    return {};
 ...
 */
vector<uint32_t> naive2(const string& txt, const string& patt) {
  const uint32_t M = patt.length(), N = txt.length();
  if (!N || !M || N < M)
    return {};
...
vector<uint32_t> KMP(const string& txt, const string& patt) {
  const uint32_t N = txt.length(), M = patt.length();
  if (!N || !M || N < M)
    return {};

  uint32_t lps[M]; // 'lps' indicates longest proper prefix which is also suffix
  lps[0] = 0; // lps[0] is always 0

  function<void(void)> computeLPS = [&]() {
    uint32_t len = 0;  // length of the previous longest prefix suffix
    uint32_t i = 1;
...
vector<uint32_t> RobinKarp(const string& txt, const string& patt, const uint32_t q) { // q : a prime number
  const uint32_t M = patt.length(), N = txt.length();
  if (!N || !M || N < M)
    return {};

  uint32_t p = 0;  // hash value for pattern
...
int main(int argc, char** argv) {
  ...
  vector<uint32_t> res = naive(txt, patt);
  for (auto i : res)
    cout << i << " ";
  cout << endl;
