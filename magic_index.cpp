/**
A magic index in an array A[0...n-1] is defined to be an index such that A[i] = i. Given a sorted array, write a method to find a magic index, if one exists, in array A.
...
int32_t magicIndex(const vector<uint32_t>& A) {
  ...
  function<int32_t(const uint32_t,const uint32_t)> solve =
    ...
    const uint32_t new_end = min(mid-1, t_idx); // optimization: array is sorted so when array[mid] < mid-1, magic index can only appear before index array[mid]
