...
vector<vector<int>> subsetsWithDup_iter(vector<int>& S) {
  ...
  return {ret.begin()+1, ret.end()}; // as ret[0] is empty
}
...
int main(int argc, char** argv) {
  ...
  cout << "Subsets of {3,2,1} are:" << endl;
  for (const auto& i : res) {
    for (const auto j : i) // "auto" works on empty vectors too, drawback??
      cout << j << " ";
    cout << endl;
  }
  ...
  cout << "Subsets of {1,2,2} are:" << endl;
  for (auto& i : res) {
    for (auto j : i)
      cout << j << " ";
    cout << endl;
