...
bool isRepeat(const string& s) {
  ...
  int32_t d = len-prefix[len-1], idx = d-1, curr_res = 0;
  if (d < 2)
    return false;
...
uint32_t longestUniqueSubstrLen(const string& s) {
  ...
  for (int32_t i = len-1; i >= 0; --i) {
    for (uint32_t j = i; j < len; ++j) {
      ...
      const uint32_t sub_len = i2j.length();
      bool no_repeat = true;

      for (uint32_t k = 0; k < sub_len; ++k) {
        for (uint32_t l = k+1; l < sub_len; ++l) {
          if (i2j[k] == i2j[l]) {
            no_repeat = false;
            break;
          }
...
*/
uint32_t longestUniqueSubstrLen(const string& s) { // O(n)
  const uint32_t len = s.length();
  if (!len)
    return 0;

  int32_t visited[MAX_CHAR];
  for (auto& i : visited)
    i = -1; /* -1 is used to indicate that character has not been visited yet. */
  visited[s[0]] = 0;

  uint32_t curr_len = 1; // length of current substring
  uint32_t ret = 1;
...
int32_t firstNonRepeating(char* str) {
  Data d[MAX_CHAR];
