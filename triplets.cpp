 ...
 */
bool isTriplet(int32_t a[], const uint32_t n) { // O(n2)
  ...
  for (int32_t i = n-1; i > 1; --i) {
    ...
    uint32_t l = 0; // index of the first element in arr[0..i-1]
    uint32_t r = i-1; // index of the last element in arr[0..i-1]
    while (l < r) {
      // A triplet found
      if (a[l]+arr[r] == a[i])
        return true;
...
/*
1) Create an auxiliary array smaller[0..n-1]. smaller[i] should store the index of a number which is smaller than a[i] and is on left side of a[i]. smaller[i] should contain -1 if there is no such element.
2) Create another auxiliary array greater[0..n-1]. greater[i] should store the index of a number which is greater than a[i] and is on right side of a[i]. greater[i] should contain -1 if there is no such element.
...
Triplet findTriplet(const int32_t a[], const uint32_t n) {
  ...
  // Create another array that will store index of a greater element on right side. If there is no greater element on right side, then greater[i] will be -1.
  int32_t greater[n];
